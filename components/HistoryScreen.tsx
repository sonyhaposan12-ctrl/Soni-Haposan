...
const HistoryScreen: React.FC<HistoryScreenProps> = ({ sessions, onBack, onClear, translations }) => {
  ...
  return (
    <>
      {selectedSession && isTranscriptModalOpen && (
          <TranscriptModal
              isOpen={isTranscriptModalOpen}
              onClose={() => setIsTranscriptModalOpen(false)}
              conversation={selectedSession.conversation}
              appMode={selectedSession.mode}
              jobTitle={selectedSession.jobTitle}
      ...
      <div className="flex flex-col h-full text-white p-4 sm:p-6 md:p-8 animate-fade-in">
        <div className="flex justify-between items-center mb-6 flex-shrink-0">
          ...
          >
            &larr; {translations.backToWelcome}
          </button>
        </div>
        ...
        {sessions.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center text-gray-400">
              ...
              <h2 className="text-2xl font-semibold text-gray-200">{translations.noSavedSessions}</h2>
              <p>{translations.noSavedSessionsDescription}</p>
          </div>
        ) : (
          <div className="flex-1 flex flex-col md:flex-row gap-6 overflow-hidden">
            ...
            <div className="w-full md:w-1/3 flex flex-col bg-gray-900/50 rounded-lg border border-gray-700">
              <div className="p-4 border-b border-gray-700 flex justify-between items-center">
                  <h2 className="text-xl font-bold">{translations.pastInterviews}</h2>
                  <button 
                      ...
                      aria-label={translations.clearAllSessions}
                  >
                      ...
                      <li key={session.id}>
                          ...
                          >
                            <div>
                                  ...
                                  <p className="text-sm text-gray-400">{session.companyName || translations.noCompany}</p>
                  ...
                  <>
                      <div className="p-4 border-b border-gray-700 flex-shrink-0 flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-sm text-gray-500 capitalize">{selectedSession.mode} {translations.mode} &middot; {selectedSession.date}</p>
                        </div>
                        <button 
                        ...
                        >
                            ...
                            <span>{translations.viewTranscript}</span>
                        </button>
                      </div>
                      <div className="flex-1 overflow-y-auto p-4">
                          <h3 className="text-xl font-bold mb-4 text-cyan-400">{translations.interviewTranscript}</h3>
                          <div className="bg-gray-800/50 rounded-lg p-2 max-h-96 overflow-y-auto">
                            <Conversation conversation={selectedSession.conversation} isProcessing={false} appMode={selectedSession.mode} translations={translations} />
                          </div>
                          <hr className="my-6 border-gray-700" />
                          <h3 className="text-xl font-bold mb-4 text-cyan-400">{translations.finalSummary}</h3>
                          <div className="bg-gray-800/50 rounded-lg p-6 text-gray-200 markdown-content" dangerouslySetInnerHTML={{ __html: parseMarkdown(selectedSession.summary) }}></div>
                      </div>
                  </>
              ) : (
                  <div className="flex-1 flex items-center justify-center text-gray-500">
                      <p>{translations.selectSessionPrompt}</p>
                  </div>
              )}
            </div>
          </div>
        ...
        {showConfirm && (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
              <div className="bg-gray-800 border border-gray-600 rounded-lg p-8 shadow-2xl max-w-sm text-center">
                  <h2 className="text-2xl font-bold mb-2">{translations.confirmClearTitle}</h2>
                  <p className="text-gray-400 mb-6">{translations.confirmClearDescription}</p>
